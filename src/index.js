...
function updateHeroPerspective(event) {
	if(window.pageYOffset <= window.innerHeight && !heroWaitingRefresh) {
		if(event){
			...
			if(event.alpha !== undefined) clientAlpha = event.alpha/360;                  // alpha [0,360]   -->[0,1]
			if(event.beta !== undefined) clientBeta = ((event.beta + 180)/360 - 0.625)%1; // beta  [-180,180]-->[0,1]
			if(event.gamma !== undefined) clientGamma = ((event.gamma + 90)/180 - 0.5)%1; // gamma [-90,90]  -->[0,1]
		...
		window.requestAnimationFrame(() => {
			perspectiveX = 0
			+ window.innerWidth/2 
			+ 1000 * ((clientAlpha - clientAlphaOrig + 0.5) % 1 - 0.5) * 2
			+ 1000 * ((clientGamma - clientGammaOrig + 0.5) % 1 - 0.5) * 2
			+ window.innerWidth/50 * Math.atan((window.innerWidth/2 - mouseX) * 2 * Math.PI / window.innerWidth);
			
			perspectiveY = 0
			+ window.pageYOffset 
			+ window.innerHeight / 4 
			+ 500 * Math.atan(((clientBeta - clientBetaOrig + 0.5) % 1 - 0.5) * 2)
